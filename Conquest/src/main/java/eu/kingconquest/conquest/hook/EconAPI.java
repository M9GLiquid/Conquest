...
public class EconAPI{
	...
	public static boolean createBank(Player player, World world){
		if (Validate.notNull(econ)){
			...
			return Vault.econ.createBank(player.getUniqueId().toString(), player).transactionSuccess();
		}
		return false;
	}
	...
	public static boolean createAccount(UUID uuid){
		if (Validate.notNull(econ)){
			...
			return Vault.econ.createPlayerAccount(uuid.toString());
		}
		return false;
    }
	...
	public static boolean createAccount(Player player){
		if (Validate.notNull(econ)){
			...
			return Vault.econ.createPlayerAccount(player);
		}
		return false;
	}
	...
	public static boolean bankExist(UUID uuid, World world){
		if (Validate.notNull(econ)){
			...
			return Vault.econ.isBankOwner(uuid.toString(), uuid.toString()).transactionSuccess();
		}
		return false;
    }
	...
	public static boolean accountExist(Player player){
		if (Validate.notNull(econ)){
			...
			return Vault.econ.hasAccount(player);
		}
		return false;
	}
	...
	public static boolean accountExist(UUID uuid){
		if (Validate.notNull(econ)){
			...
			return Vault.econ.hasAccount(uuid.toString());
		}
		return false;
    }

	@SuppressWarnings("all")
	public static void addFunds(Player player, Double amount) {
		if (Validate.notNull(econ)){
	...
	public static boolean addFunds(UUID uuid, Double amount){
		if (Validate.notNull(econ)){
			...
			return Vault.econ.depositPlayer("kingdom-" + uuid.toString(), amount).transactionSuccess();
		}
		return false;
    }
