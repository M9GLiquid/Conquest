...
public class YmlStorage extends YamlConfiguration{
	...
	public static void registerFiles(){
		...
		try(Stream<Path> paths = Files.list(Paths.get(Main.getInstance().getDataFolder() + File.separator + "Data" + File.separator + "Users"))) {
		...
		try(Stream<Path> paths = Files.list(Paths.get(Main.getInstance().getDataFolder() + File.separator + "Data" + File.separator + "Rewards"))) {
			paths.filter(Files::isRegularFile).forEach(filePath -> {
