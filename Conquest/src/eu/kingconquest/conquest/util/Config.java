		...
		try{
			getPathSection(kingdoms, world.getName()).forEach(uniqueID ->{
				...
				Kingdom kingdom = new Kingdom(kingdoms.getString(world.getName() + "." + uniqueID + ".Name"),
						...
						uniqueID,
						kingdoms.getLocation(world, world.getName() + "." + uniqueID + ".Location"),
						kingdoms.getLocation(world, world.getName() + "." + uniqueID + ".Spawn"),
				...
				getPathSection(users, world.getName()).forEach(kUUID->{
					if (kUUID.equals(kingdom.getUUID().toString())){
						getPathSection(users, world.getName() + "." + kUUID).forEach(uUUID->{
							...
							if (!Validate.isNull(Bukkit.getPlayer(UUID.fromString(uUUID)))){
		...
		try {
			...
			getPathSection(towns, world.getName()).forEach(uniqueID ->{
				...
				new Town(towns.getString(world.getName() + "." + uniqueID + ".Name")
						,uniqueID
						,towns.getLocation(world, world.getName() + "." + uniqueID + ".Location")
						,towns.getLocation(world, world.getName() + "." + uniqueID + ".Spawn")
						,Kingdom.getKingdom(UUID.fromString(towns.getString(world.getName() + "." + uniqueID + ".Owner")), world));
			});
			return true;
		}catch(Exception e){
			e.printStackTrace();
		...
		try {
			getPathSection(villages, world.getName()).forEach(uniqueID->{
				...
				for (Village village : Village.getVillages())
					if (village.getUUID().toString().equals(uniqueID)) //If Village already Exists
						return;
				if (villages.isSet((world.getName() + "." + uniqueID + ".Parent")))
					parent = Town.getTown(UUID.fromString(villages.getString(world.getName() + "." + uniqueID + ".Parent")), world);
				else
					parent = null;
				Village village = new Village(
						villages.getString(world.getName() + "." + uniqueID + ".Name"),
						uniqueID, 
						villages.getLocation(world, world.getName() + "." + uniqueID + ".Location"),
						villages.getLocation(world, world.getName() + "." + uniqueID + ".Spawn"),
						Kingdom.getKingdom(UUID.fromString(villages.getString(world.getName() + "." + uniqueID + ".Owner")), world),
						Kingdom.getKingdom(UUID.fromString(villages.getString(world.getName() + "." + uniqueID + ".PreOwner")), world),
						parent);
	...
	public static boolean saveVillages(World world){
		...
		try {
			Village.getVillages(world).forEach(village->{
				...
				if (!Validate.isNull(village.getPreOwner()))
					villages.set(world.getName() + "." + village.getUUID().toString() + ".PreOwner", village.getPreOwner().getUUID().toString());
				if (village.hasParent())
